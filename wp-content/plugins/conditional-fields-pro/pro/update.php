...
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	...
	WPCF7CF_PLUGIN_DIR.'/conditional-fields.php',
	WPCF7CF_PLUGIN_NAME
);


if( is_admin() ) {
	add_action('in_plugin_update_message-'.WPCF7CF_PLUGIN_NAME.'/conditional-fields.php', 'wpcf7cf_modify_plugin_update_message', 99, 2 );
}
...
function wpcf7cf_modify_plugin_update_message( $plugin_data, $response ) {
	...
	if ($response->package === 'EXPIRED_KEY') {
		echo '</p><p>' . sprintf( __('<strong>License key expired</strong>. Please <a href="%s" target="_blank">renew your license</a>.', 'wpcf7cf'), 'https://shop.bdwm.be/conditional-fields-pro/' );
		return;
	}
}
