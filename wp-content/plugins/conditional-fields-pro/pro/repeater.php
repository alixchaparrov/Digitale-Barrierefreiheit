			...
			foreach ($form_parts as $form_part) {
				if (substr($form_part,0,10) == '[repeater ') {
					...
					echo str_replace( "\n", '', '
							...
							<p class="wpcf7cf_repeater_controls">
								<input type="hidden" name="'.$tag_id.'_count" value="">
								<span class="wpcf7cf_add-container">
									<button type="button" class="wpcf7cf_add">'.array_pop($add_button_text).'</button>
								</span>
								<span class="wpcf7cf_remove-container">
									<button type="button" class="wpcf7cf_remove">'.array_pop($remove_button_text).'</button>
								</span>
							</p>
						</'.array_pop($stack).'>
					');
